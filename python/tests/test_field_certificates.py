...
def test_rocksalt_zero_width_mirror():
    """The rocksalt layout (single anchor c = 5, bulk 6) is fully identified
    in range — Lean `corrected_exact_rocksalt`: zero gap, zero width."""
    cert = check_anchor_admissibility(-4123, structure="rocksalt")
    assert cert.tier == "error_field"
    assert cert.gap_coordination is None
    assert cert.bracket_width_scaled == 0
    assert cert.identification_ref == THEOREM_REFS["anchor_identification_rocksalt"]
