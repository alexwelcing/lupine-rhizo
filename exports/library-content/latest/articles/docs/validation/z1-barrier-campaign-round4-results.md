...
- Measurement rows: `data/candidates/z1/measurements.jsonl` (four RFC 8785 hash-chained aggregate fail rows, `tools/build_z1_measurement_rows.py`); the float64 chain has its own parallel set at `data/candidates/z1/f64/measurements.jsonl` (+ `f64/artifact-manifest.json`, built with `--source-root data/candidates/z1/f64 --result-uri-root gs://shed-489901-atlas-outputs/z1/campaign-float64`).
