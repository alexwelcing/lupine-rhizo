...
def render_feedback_sql(plan: dict[str, Any], *, d1_transactional: bool = False) -> str:
    """Render one atomic, reviewable D1 script from a feedback plan.

    ``d1_transactional=True`` omits the SQLite ``BEGIN TRANSACTION`` /
    ``COMMIT`` wrapper: D1 rejects explicit transactions ("use
    state.storage.transaction() instead") while plain statement batches
    already run atomically per request. The local staging path (sqlite3)
    keeps the wrapper so its rollback semantics stay intact.
    """
    statements = ["-- Generated by tools/nightly_ontology_feedback.py"]
    if not d1_transactional:
...
def main() -> int:
    ...
    parser.add_argument(
        "--d1",
        action="store_true",
        help="render for D1 execution: omit the explicit BEGIN/COMMIT wrapper D1 rejects",
    )
    ...
    args.out_sql.write_text(render_feedback_sql(plan, d1_transactional=args.d1), encoding="utf-8")
