# Round-5 preregistration v2 — powered sharp-license correction trial

> **Status:** FROZEN FOR REGISTRATION. The registration time is the
> review-remediation registration event, `2026-08-04T14:05:04Z`.
...
`data/candidates/round5_elastic_panel-selection.v2.lock.json`. That contract is the
...
The selector allocates each class quota across complete calibration groups. For
every exact grouping tuple its selected occupancy is either zero or at least
five, so every selected structure has at least four other panel structures for
leave-one-out calibration. Allocation is solved as an exact deterministic
integer feasibility problem; if the class quota cannot be represented without
an underfilled selected tuple, materialization REFUSES. Before dispatch, the
materializer must also prove from the locked references—not from model
outputs—that at least 16 distinct selected structures in every class×property
have four same-tuple peers with a provenance-accepted reference. A failure is
`STRUCTURALLY_UNDERPOWERED_REFUSE`, not permission to run an underpowered panel.
...
hulls are wider than lattice-constant hulls at the **distinct-structure**
sampling unit. Hull width is `(hi - lo) / U`. The required fields on every
candidate and result row are:
...
For each structure, class, and elastic property (`C11`, `C12`, `C44`), form one
fixed-point paired difference per registered model:
`d_m = elastic_width_m - a0_width_m`, using widths from the same held-out
structure and exact grouping tuple. A structure enters this secondary contrast
only when all four registered models have both widths; otherwise the structure
is excluded with `incomplete_registered_model_pairs`. Sort the four integer
differences and aggregate them to one structure-level difference using the
median; the even median is the arithmetic mean of the two central integers,
rounded to integer units by decimal round-half-even. Positive is wider,
negative is narrower, and exact zero is a dropped tie. Thus each structure
contributes at most one Bernoulli sign and correlated model outputs are never
counted as independent trials.

Test the directional alternative `structure median elastic width > structure
median a0 width` with a one-sided exact sign test. Control the six class×elastic
tests with Holm step-down family-wise alpha 0.1. Report distinct-structure
paired n, wider/equal/narrower counts, structure-median widths, median width
ratio when the structure-level a0 width is positive, raw p, and Holm-adjusted
p. No subgroup may be promoted to a confirmatory claim.
...
The rejected v1 registration remains immutable historical evidence and is not
executable. This v2 registration is a new campaign identity created before any
Round-5 outcome was inspected; it repairs the deterministic calibration-group
occupancy and secondary-endpoint sampling-unit defects found by independent
review.

The v2 CampaignManifest, this preregistration document, and the panel-selection
