...
- **170 build-locked theorems** in the executable vision, **376 theorem declarations** in `Materials/`, **0 `sorry` proofs**, **3661-job build green**.
...
- The measured-fields rung (`Theory/AnchoredField`,
  ...
  into per-cell field instances at two tiers: every fcc (model, material)
  cell yields a `MeasuredField 12` (closure/transfer/ranking laws), and the
  monotone-softening cells additionally yield an `ErrorField 12`
  (directional barrier laws) — 36 measured fields, 8 anchored instances,
  28 kernel-checked tier-2 refusals. The promotion gate mirrors these
  certificates in `python/lupine_distill/odf/field_certificates.py`.
