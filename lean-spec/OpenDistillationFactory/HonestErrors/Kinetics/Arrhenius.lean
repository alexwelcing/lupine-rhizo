...
/-- The report's Arrhenius rate, reusing the factory's existing hop-rate law. -/
noncomputable abbrev arrheniusRate :=
  OpenDistillationFactory.Materials.Theory.BarrierArrhenius.hopRate
...
theorem arrhenius_transport
  ...
  unfold arrheniusRate
    OpenDistillationFactory.Materials.Theory.BarrierArrhenius.hopRate
    OpenDistillationFactory.Materials.Theory.BarrierArrhenius.boltzmann
    signedBarrierError
  calc
    ...
    _ = Real.exp (-(predicted - reference) / thermalEnergy) *
        (prefactor * Real.exp (-(reference / thermalEnergy))) := by ring_nf
